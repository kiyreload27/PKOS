...
export default function MagicalCaptureDashboard() {
  ...
  const handleCapture = async () => {
    ...
    setPipelineState("capturing"); // Maps to Receiving
    ...
    try {
      ...
      if (data.captureId) {
        ...
        const sse = new EventSource(`/api/capture/${data.captureId}/events`);
        ...
        sse.onmessage = (event) => {
          ...
          } else if (payload.type === "CAPTURE_ANALYSED") {
            setPipelineState("summarizing"); // Represents Extracting
          } else if (payload.type === "ENTITY_CREATED") {
            setPipelineState("relating"); // Represents Rel. Discovery
          }
        };
